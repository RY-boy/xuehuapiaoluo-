...
- (void)viewDidLoad {
    ...
    imageView.tag = 2;
    [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(changeImageViewState) userInfo:nil repeats:YES];
#else
    //创建一个火焰的imageView
...
-(void)snowFly
{
    ...
    //获取屏幕宽度
    int MYScreen = [UIScreen mainScreen].bounds.size.width;
    //雪花的起始位置
    int x1 = round(random()%100 +1.0);
    //雪花大小的倍数
